...
export const OverviewTab = ({ ports, actions }) => {
    ...
    const toggleExpanded = (portNum) => {
        setExpandedRow(expandedRow === portNum ? null : portNum);
    };
                        ...
                        {filteredAndSortedPorts.map((port) => (
                            <React.Fragment key={port.port}>
                                <tr className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        <button
                                            onClick={() => toggleExpanded(port.port)}
                                            className="text-gray-500 hover:text-white transition-colors"
                                        >
                                            {expandedRow === port.port ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                        </button>
                                    </td>

                                    {/* Port */}
                                    <td className="p-4 font-mono text-cyan-400 font-bold">{port.port}</td>

                                    {/* Process */}
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            <div className="font-medium text-white">{port.name}</div>
                                            <button
                                    ...
                                    {/* PID */}
                                    <td className="p-4 text-gray-500 font-mono">{port.pid}</td>
                                ...
                                {(expandedRow === port.port || activeLogViewer === port.pid) && (
